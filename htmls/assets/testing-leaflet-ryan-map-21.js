...
//set the style on the polygon
	...
	//Todo: add a back-button which will send the user back to the original setting/map if clicked, and suicide once the original setting/map is achieved.
			...
			layer.on("mouseout",function(e){
				buildingCounter = countEvidence();
				if (layer.properties.ITEMNUM == buildingCounter[layer.properties.ID]){
				layer.setStyle(clearedStyle);} else {layer.setStyle(defaultStyle)};
				//changeColor(points, boundaries, layer);
					//changeStyle(feature, points);
				$("#popup-"+properties.ID).remove();
			});
...
//Add the GeoJSON to the layer, which is loaded in the <head>
	var featureLayer = L.geoJson(boundaries, {style: function(feature){ //add points?
		buildingCounter = countEvidence();
		if (feature.properties.ITEMNUM == buildingCounter[feature.properties.ID]){
			return unlockedStyle;
		} else {
			return defaultStyle;
		}
